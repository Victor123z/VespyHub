...
TOKEN = 'OTU0MzAyNjE5MTYwMDQ3NjY4.YjRJkg.YD8hyrAdiszyB25S9Opq7w8r2Bc'
LIMIT =  50
def randomString(N):
        ...
        if (phone == None or amount == None):
            ...
            try:
                ...
                if (amount > LIMIT):
                    ...
                    embed=discord.Embed(title="GOTji On The Top", color=0x00ff00, timestamp=datetime.utcnow(),)
                    embed.set_footer(text="บีมคุง")
                 #   embed.set_thumbnail(url='https://cdn.discordapp.com/attachments/934796371037405224/955980347458355200/92004028_p0.png')
                    ...
                    embed.add_field(name="บอทมีปัญหาติดต่อได้ที่", value="``` บีมคุง#7661  ```", inline=True)
                    components = ([
        [Button(label="Open At 8 April", style="3", emoji = "❤️", custom_id="BetaMax"), Button(label="Open At 8 April", style="4", emoji = "❤️", custom_id="BetaPremium"), Button(label="Open At 8 April", style="4", emoji = "❤️", custom_id="BetaDev")]
        ])
                    await ctx.reply(embed=embed)
                    await ctx.send("Offical Discord \n https://discord.gg/cQvTG96M4r")
                    await ctx.reply(components=components)
                    await ctx.send("คำสั่งเพิ่มเติม", components = [
        Select(
            placeholder = "กดเพื่อเลือกเมนู",
            options = [
                SelectOption(label="เปิดให้ใช้ 8 เมษายน", value="Max"),
                SelectOption(label="เปิดให้ใช้ 8 เมษายน", value="Vip"),
                SelectOption(label="เปิดให้ใช้ 8 เมษายน", value="Premium"),
                SelectOption(label="เปิดให้ใช้ 8 เมษายน", value="Partner")
            ]
        )
    ])
                    startall(phone, amount)
            except:
                embed=discord.Embed(title="Unsuccess Spam", description="```คุณใส่จำนวนไม่ถูกต้องหรือเกิดจากบัค \n ไม่ส่งผลต่อการยิง \n หนูยังคงยิงไปที่เบอร์ \n {phone} นะงับ```", color=0x00ff00, timestamp=datetime.utcnow())
                embed.set_footer(text="บีมคุง")
